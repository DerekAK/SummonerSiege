...
public class PlayerMovementRagdoll : PlayerMovement
{
    ...
    private void LateUpdate() {
        ...
        //HandleCameraZoom(); // needs to be late to enable/disable head renderers depending on camera distance
    }
    ...
    private void HandleCameraZoom()
    {
        ...
        if (Mathf.Abs(scrollInput) > 0.01f)
        {
            ...
            if (newCameraDistance < firstPersonCameraDistanceThreshold)
            {
                foreach(var rend in _physicsManager.CurrHeadRenderers){ rend.enabled = false; }
            }
            else 
            {
                foreach(var rend in _physicsManager.CurrHeadRenderers){ rend.enabled = true; }
            }
        
            // Apply to camera
            currentCameraDistance = Mathf.Clamp(newCameraDistance, minCameraDistance, maxCameraDistance);
    ...
    private void Roll(Vector3 moveDir)
    {
        ...
        Vector2 rollForceXY = new Vector2(moveDir.x, moveDir.z) * rollForce;

        foreach (var rb in ragdollRigidbodies)
        {
            rb.linearVelocity = new Vector3(rollForceXY.x, rb.linearVelocity.y, rollForceXY.y);
        }
        _rb.linearVelocity = new Vector3(rollForceXY.x, _rb.linearVelocity.y, rollForceXY.y);
    }
    
    public override void AnimationEvent_EndRoll()
    {
